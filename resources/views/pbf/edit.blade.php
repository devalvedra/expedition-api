...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white">Edit PBF</h1>
        <a href="{{ route('pbf.index') }}" class="text-sm text-blue-600 hover:text-blue-800 dark:text-blue-400">&larr; Kembali</a>
    </div>

    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <form method="POST" action="{{ route('pbf.update', $pbf->id) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="kode_pbf" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Kode PBF</label>
                <input type="text" name="kode_pbf" id="kode_pbf" value="{{ old('kode_pbf', $pbf->kode_pbf) }}" required
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_pbf" id="nama_pbf" value="{{ old('nama_pbf', $pbf->nama_pbf) }}" required
                       class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">
                @error('nama_pbf')
                    <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <textarea name="alamat" id="alamat" rows="3" required
                          class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">{{ old('alamat', $pbf->alamat) }}</textarea>
                @error('alamat')
                    <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="number" name="lat" id="lat" value="{{ old('lat', $pbf->lat) }}" required step="any" min="-90" max="90"
                           class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    @error('lat')
                        <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="lng" id="lng" value="{{ old('lng', $pbf->lng) }}" required step="any" min="-180" max="180"
                           class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    @error('lng')
                        <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
            ...
            <div class="mb-6">
                ...
                @if($pbf->image_path)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $pbf->image_path) }}" alt="{{ $pbf->nama_pbf }}" class="h-24 w-24 object-cover rounded-lg border border-gray-300 dark:border-gray-600">
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Gambar saat ini. Upload baru untuk mengganti.</p>
                    </div>
                @endif
